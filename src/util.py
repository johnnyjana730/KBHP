...
def topk_eval(args, user_triplet_set, model, user_list, train_record, eval_record, test_record, item_set, k_list, batch_size, mode = 'test'):
    ...
    for user in user_list:
        ...
        if user in ref_user:
            ...
            while start + batch_size <= len(test_item_list):
                ...
                items, scores = model.get_scores(*get_feed_dict_top_k(args, model, user_list_tmp, item_list, labels_list, user_triplet_set))
...
def get_feed_dict_top_k(args, model, user_list, item_list, labels_list, ripple_set):
    users = torch.LongTensor(user_list)
    items = torch.LongTensor(item_list)
    labels = torch.LongTensor(labels_list)
    memories_h, memories_r, memories_t = [], [], []
    for i in range(args.n_hop):
        memories_h.append(torch.LongTensor([ripple_set[user][i][0] for user in user_list]))
        memories_r.append(torch.LongTensor([ripple_set[user][i][1] for user in user_list]))
        memories_t.append(torch.LongTensor([ripple_set[user][i][2] for user in user_list]))
    if args.use_cuda:
        users = users.cuda()
        items = items.cuda()
        labels = labels.cuda()
