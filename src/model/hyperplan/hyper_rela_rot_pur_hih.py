...
class hpNet_rela_rot_pur_hih(nn.Module):
    def __init__(self, args, n_entity, n_relation):
        super(hpNet_rela_rot_pur_hih, self).__init__()
        ...
        self.rela_plus_emb_matrix = nn.Embedding(self.n_relation + 2, self.dim)

        if self.manifold.name == 'Hyperboloid':
            self.rela_plus_emb_matrix.weight.data =  torch.rand((self.n_relation + 2, self.dim)) 
        else:
            self.rela_plus_emb_matrix.weight.data =  torch.rand((self.n_relation + 2, self.dim)) 

        if self.manifold.name == 'Hyperboloid':
            self.relation_emb_matrix = nn.Embedding(self.n_relation + 2, (self.dim) * (self.dim))

            self.relation_emb_matrix.weight.data =  torch.randn((self.n_relation + 2, (self.dim) * (self.dim)))
        else:
            self.relation_emb_matrix = nn.Embedding(self.n_relation + 2, self.dim * self.dim)

            self.relation_emb_matrix.weight.data =  torch.randn((self.n_relation + 2,(self.dim) * (self.dim)))

        self.rel_diag_matrix = nn.Embedding(self.n_relation + 2, self.dim)
        self.rel_diag_matrix.weight.data = 2 * torch.rand((self.n_relation + 2,  self.dim)) - 1.0

        self.rel_diag_2_matrix = nn.Embedding(self.n_relation + 2, self.dim)
        self.rel_diag_2_matrix.weight.data = 2 * torch.rand((self.n_relation + 2, self.dim)) - 1.0
        
        self.context_vec = nn.Embedding(self.n_relation + 2,  self.dim)
        if self.manifold.name == 'Hyperboloid':
            self.context_vec.weight.data =  torch.randn((self.n_relation + 2, self.dim))
        else:
            self.context_vec.weight.data = self.init_size * torch.randn((self.n_relation + 2, self.dim))
        ...
        self.one_like_bahop =  nn.Parameter(torch.ones_like(torch.randn(args.batch_size, args.n_hop + 1)), requires_grad = False)
    ...
    def forward(self, users: torch.LongTensor, items: torch.LongTensor, labels: torch.LongTensor, memories_h: list, memories_r: list, memories_t: list):
        ...
        self.h0_diag_emb = self.rel_diag_matrix(torch.LongTensor([self.n_relation + 1] * (int(memories_h[i].shape[0]) * int(memories_h[i].shape[1]))).cuda())
        self.h0_diag_2_emb = self.rel_diag_2_matrix(torch.LongTensor([self.n_relation + 1] * (int(memories_h[i].shape[0]) * int(memories_h[i].shape[1]))).cuda())
        self.h0_plus_emb = self.rela_plus_emb_matrix(torch.LongTensor([self.n_relation + 1] * (int(memories_h[i].shape[0]) * int(memories_h[i].shape[1]))).cuda())
        self.h0_daig_att = self.context_vec(torch.LongTensor([self.n_relation + 1] * (int(memories_h[i].shape[0]) * int(memories_h[i].shape[1]))).cuda())
    ...
    def _key_hper_addressing(self):
        ...
        h_emb_sp = self.t_emb_list[0].shape
        batch_men, men, dim = h_emb_sp[0], h_emb_sp[1], h_emb_sp[2]

        hyp_h_emb = self.manifold.proj_tan0(self.h_emb_list[0].view(-1, dim), self.cur[0])

        h_emb_rot_q = givens_rotations(self.h0_diag_emb.view(-1, dim), hyp_h_emb.view(-1, dim)).view((-1, 1, dim))
        h_emb_ref_q = givens_rotations(self.h0_diag_2_emb.view(-1, dim), hyp_h_emb.view(-1, dim)).view((-1, 1, dim))

        cands = torch.cat([h_emb_rot_q, h_emb_ref_q], dim=1)
        att_weights = torch.sum(self.h0_daig_att.view(-1, 1, dim) * cands * self.scale, dim=-1, keepdim=True)
        att_weights = F.softmax(att_weights, dim=1)

        att_q = torch.sum(att_weights * cands, dim=1)
        lhs = self.manifold.proj_tan0_exp(att_q, self.cur[0])

        hyp_r_pl_emb = self.manifold.proj_tan0_exp(self.h0_plus_emb.view(-1, dim), self.cur[0])
        ...
        hyp_t_emb = self.manifold.proj_tan0_exp(self.h_emb_list[0].view(-1, dim), self.cur[0])
